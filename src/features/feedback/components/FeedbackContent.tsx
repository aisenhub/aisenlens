...
export default function FeedbackContent() {
  ...
  const submitFeedback = async () => {
    ...
    setErrorMessage("");
    try {
      await createFeedbackRequest({ kind, title: normalizedTitle, content: normalizedContent });
      setTitle("");
      setContent("");
      ...
      <SurfaceCard className="p-6 sm:p-8">
        ...
        <div className="mb-6 grid grid-cols-2 gap-2 sm:grid-cols-4">
          {(Object.keys(FEEDBACK_KINDS) as FeedbackKind[]).map((feedbackKind) => {
            ...
            return (
              <Button key={feedbackKind} type="button" variant="outline" onClick={() => { setKind(feedbackKind); setErrorMessage(""); }} className={`h-auto min-w-0 items-start whitespace-normal rounded-xl px-3 py-3 text-left ${isSelected ? "border-accent/40 bg-accent/10 text-white hover:bg-accent/15 hover:text-white" : "border-border text-text-dim hover:border-border-mid hover:bg-white/4 hover:text-white"}`}>
                <span className="min-w-0"><span className="block break-words text-sm font-medium">{option.label}</span><span className="mt-1 block break-words text-xs leading-relaxed text-text-muted">{option.description}</span></span>
              </Button>
            );
          })}
        </div>
        ...
        <Button type="button" onClick={() => void submitFeedback()} disabled={isSubmitting || title.trim().length < 3 || content.trim().length < 10} className="h-11 w-full rounded-xl bg-accent font-semibold text-white hover:bg-accent/90">{isSubmitting ? "正在提交…" : "提交反馈"}</Button>
      </SurfaceCard>
    </div>
  );
}
