...
test("Overview / Analyze 在常用视口下保持可导航且研究入口可见", { timeout: 120_000 }, async (context) => {
  ...
  for (const viewport of [{ width: 1440, height: 900 }, { width: 1280, height: 720 }, { width: 768, height: 1024 }, { width: 390, height: 844 }]) {
    ...
    await until(async () => (await evaluate(client, sessionId, "[...document.querySelectorAll('button')].some((element) => element.textContent?.includes('深拆'))")), `深拆入口未出现：${viewport.width}x${viewport.height}`)
    ...
    await until(async () => (await evaluate(client, sessionId, "[...document.querySelectorAll('button')].some((element) => element.textContent?.trim() === 'Shots')")), `Shots 入口未出现：${viewport.width}x${viewport.height}`)
