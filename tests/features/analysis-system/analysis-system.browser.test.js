...
test("P1 只读盘点当前 IndexedDB schema 与 Marker v2 字段形态", { timeout: 120_000 }, async (context) => {
  assert.ok(browserPath, "未找到 Chrome 或 Edge；可通过 AISENLENS_CHROME_PATH 指定浏览器路径。")
  ...
  assert.equal(inventory.version, 18)
  ...
  assert.ok(inventory.stores.includes("annotation-markers"))
...
test("P3 Focus 选段范围只排入范围内镜头", { timeout: 120_000 }, async (context) => {
  ...
  await navigateToProject(client, sessionId, serverPort, projectId, `stage=analyze&view=shots&scopeKind=transient-range&fromUs=1200000&toUs=3200000&targetKind=range&targetId=${rangeId}`)
