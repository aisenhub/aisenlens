...
test("P1 UI 保存失败保留 dirty 与基线，清除故障后可重试成功", { timeout: 120_000 }, async (context) => {
  ...
  await navigateToProject(client, sessionId, serverPort, projectId, "workspace=analysis&view=shots")
...
test("P2 模板 dirty draft 与 Apply/Undo/Redo/重载链路", { timeout: 120_000 }, async (context) => {
  ...
  const { projectId } = await seedProject(client, sessionId, 1)
  await navigateToProject(client, sessionId, await evaluate(client, sessionId, "location.port"), projectId, "workspace=preparation&view=media")
...
test("P3 Focus 选段范围只排入范围内镜头", { timeout: 120_000 }, async (context) => {
  ...
  await navigateToProject(client, sessionId, serverPort, projectId, `workspace=analysis&view=shots&mode=range&scopeKind=transient-range&fromUs=1200000&toUs=3200000&targetKind=range&targetId=${rangeId}`)
