...
export default function ProjectLibrary({ onOpenProject }: ProjectLibraryProps) {
  ...
  const ProjectCard = ({ project: projectRecord }: { project: ProjectRecord }) => {
    ...
    <SurfaceCard className="group relative cursor-pointer overflow-hidden transition-colors hover:border-accent/35" onClick={() => onOpenProject(project)}>
      ...
      <div className="p-4"><div className="mb-2 flex items-start justify-between gap-2"><h3 className="line-clamp-2 font-display text-sm font-bold leading-snug text-white transition-colors group-hover:text-accent">{project.title}</h3>{mediaStatusText(project.media.status) && <span className="shrink-0 rounded-full border border-amber-400/20 bg-amber-400/10 px-1.5 py-0.5 font-mono text-[10px] text-amber-200">{mediaStatusText(project.media.status)}</span>}</div><p className="mb-3 truncate text-xs text-text-muted">{project.media.source?.name ?? "尚未添加本地视频"}</p><div className="flex items-center justify-between font-mono text-xs text-text-muted"><span>{project.shots ? `${project.shots} 镜头` : "未开始"}</span><span>{project.notes ? `${project.notes} 批注` : "—"}</span><span>{formatUpdatedAt(project.updatedAt)}</span></div></div>
    </SurfaceCard>
    );
  };
