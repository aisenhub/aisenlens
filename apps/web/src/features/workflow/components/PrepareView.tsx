    ...
    <main className="min-h-0 flex-1 overflow-y-auto bg-bg px-4 py-5 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="space-y-4">
          ...
          <section className="border border-border bg-bg-panel p-4"><div className="mb-4 flex items-start gap-3"><Map className="mt-0.5 size-4 text-accent" /><div><h2 className="text-sm font-medium text-text-base">建立镜头地图</h2><p className="mt-1 text-xs leading-5 text-text-muted">自动分镜只提出候选区间；应用前会在校准阶段复核并创建恢复快照。</p></div></div><AutoShotControlPanel settings={settings} resolved={resolved} presets={presets} record={record} isActive={isActive} error={error} onChange={onChange} onStart={onStart} onPause={onPause} onRestart={onRestart} />{record?.status === "completed" && <Button type="button" size="sm" onClick={onGoToCalibrate} className="mt-3 h-8 w-full gap-2 bg-accent text-white hover:bg-accent/90">应用分镜<ArrowRight className="size-3.5" /></Button>}</section>
        </div>
      </div>
    </main>
  )
