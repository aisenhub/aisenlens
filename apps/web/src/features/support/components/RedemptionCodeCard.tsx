...
import { redeemAisenLensCode } from "../../../services/aisenhub/redemption"
...
function getRedemptionErrorMessage(error: unknown) {
  ...
  if (
    message.includes("campaign is unavailable") ||
    message.includes("campaign has reached")
  )
    return "该兑换活动暂不可用。"

  if (message.includes("invalid redemption code"))
    return "兑换码无效，请检查后重试。"
...
export default function RedemptionCodeCard({
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await redeemAisenLensCode(normalizedCode)
      setCode("")
      toast.success("兑换成功，平台权益已更新。")
    } catch (error) {
    ...
    <SurfaceCard className="mb-10 p-6 sm:p-8">
      ...
      >
        <Input
          ...
          placeholder="例如 AL-XXX"
          autoComplete="off"
          maxLength={23}
          className="h-11 flex-1 border-border bg-bg-input px-4 font-mono text-white placeholder:text-text-muted"
        />
        <Button
          type="submit"
          disabled={isSubmitting}
          className="h-11 rounded-xl bg-accent px-7 font-semibold text-white hover:bg-accent/90"
        >
