...
export default function CalibrationWorkspace({ projectId, projectUpdatedAt, mediaIdentity, mediaSource, videoUrl, frameRate, totalFrames, durationSeconds, task, formalShots, previewProps, controlsProps, onApply, onBack }: CalibrationWorkspaceProps) {
  ...
  return <main className="flex min-h-0 flex-1 flex-col overflow-hidden bg-bg-deep text-text-base">
    ...
    <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[15rem_minmax(0,1fr)_17rem]">
      <aside className="order-2 flex min-h-0 flex-col border-t border-border bg-bg-panel lg:order-1 lg:border-r lg:border-t-0">
      ...
      <aside className="order-3 min-h-0 overflow-y-auto border-t border-border bg-bg-panel p-3 lg:border-l lg:border-t-0">
        ...
        <div className="mt-5 border-t border-border pt-3"><h2 className="text-xs font-medium">边界检查</h2>{selectedBoundary ? <div className="mt-2 space-y-2"><p className="font-mono text-[11px] text-text-muted">{formatTime(selectedBoundary.frame, frameRate)} · 第 {selectedBoundary.frame} 帧</p><BoundaryFramePair projectId={projectId} sourceUrl={videoUrl ?? ""} mediaFingerprint={mediaSource} timebase={frameTimeline.timeline} boundaryFrame={selectedBoundary.frame} /><div className="grid grid-cols-2 gap-1.5"><Button type="button" variant="outline" size="sm" onClick={() => dispatch({ type: "moveBoundary", boundaryId: selectedBoundary.id, frame: selectedBoundary.frame - 1 })}>前移一帧</Button><Button type="button" variant="outline" size="sm" onClick={() => dispatch({ type: "moveBoundary", boundaryId: selectedBoundary.id, frame: selectedBoundary.frame + 1 })}>后移一帧</Button><Button type="button" variant="outline" size="sm" onClick={() => dispatch({ type: "moveBoundary", boundaryId: selectedBoundary.id, frame: frameTimeline.timeline ? timestampToFrame(frameTimeline.timeline, controlsProps.currentTime) : selectedBoundary.frame })}>移动到当前帧</Button><Button type="button" variant="destructive" size="sm" onClick={() => { dispatch({ type: "removeBoundary", boundaryId: selectedBoundary.id }); setSelectedBoundaryId(null) }}><Trash2 className="size-3" />合并前后镜头</Button></div></div> : <p className="mt-2 text-[10px] leading-4 text-text-muted">点击下方边界列表选择切点，查看前镜末帧与后镜首帧。</p>}{draft.boundaries.map((boundary) => <button key={boundary.id} type="button" onClick={() => { setSelectedBoundaryId(boundary.id); moveToFrame(boundary.frame) }} className={`mt-2 flex w-full items-center justify-between rounded border px-2 py-1.5 text-left text-[10px] ${selectedBoundaryId === boundary.id ? "border-accent/50 bg-accent/10" : "border-border/70"}`}><span className="font-mono">{formatTime(boundary.frame, frameRate)}</span><span className="text-text-muted">{boundary.source === "manual" ? "人工" : "检测"}</span></button>)}</div>
