...
function ShotList({ className, shots, groups, collapsedGroupIds, activeShotIndex, filters, onFiltersChange, completionByShotId, selectedShotIds, isSelectingShots, shotNotes, shotFields, screenshotIdsByShotId, primaryScreenshotIdsByShotId, markers, onLocateShot, onSelectionChange, onToggleGroup, onSelectGroup, onPlayGroup, onDeleteGroup, onPlayShot, onDeleteShot, manualSplitDisabledReason, onSplitAtPlayhead }: ShotListProps) {
  ...
  const matchingShotIds = useMemo(() => findMatchingShotIds({ shots, groups, notesByShotId: shotNotes, fieldsByShotId: shotFields, screenshotIdsByShotId, primaryScreenshotIdsByShotId, markers }, filters), [filters, groups, markers, primaryScreenshotIdsByShotId, screenshotIdsByShotId, shotFields, shotNotes, shots]);
    ...
    <div className="flex-1 overflow-y-auto">
      {shots.map((shot, index) => {
        ...
        return <div key={shot.id}>
          {isGroupStart && group && groupHasMatch && <div style={{ contentVisibility: "auto", containIntrinsicSize: "0 36px" }} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-1 border-b border-accent/15 bg-accent/6 px-2 py-1.5"><Button type="button" variant="ghost" size="icon-xs" aria-label={isCollapsed ? "展开分组" : "折叠分组"} onClick={() => onToggleGroup(group.id)} className="text-accent hover:bg-accent/10">{isCollapsed ? <ChevronRight /> : <ChevronDown />}</Button><Button type="button" variant="ghost" size="sm" onClick={() => onSelectGroup(group.id, index)} title={group.title} className="h-7 min-w-0 justify-start truncate px-1 text-left editor-body text-accent hover:bg-accent/10">{group.title}</Button><DropdownMenu><DropdownMenuTrigger render={<Button type="button" variant="ghost" size="icon-xs" aria-label={`分组 ${group.title} 的更多操作`} className="text-accent hover:bg-accent/10" />}><MoreHorizontal /></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-32"><DropdownMenuItem onClick={() => onPlayGroup(group.id)}><Play />播放分组</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => onDeleteGroup(group.id)}><Trash2 />删除分组</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>}
