...
import { timestampToFrameIndex } from "../services/mediaFrameTimeService";
...
function frameSlots({ durationSeconds, frameRate, presentationTimestamps, pixelsPerSecond, visibleStart, visibleEnd }: Pick<FrameThumbnailStripProps, "durationSeconds" | "frameRate" | "presentationTimestamps" | "pixelsPerSecond" | "visibleStart" | "visibleEnd">): number[] {
  ...
  const maximumFrame = Math.max(0, (presentationTimestamps?.length ?? Math.floor(durationSeconds * frameRate)) - 1);
  return [...new Set(Array.from({ length: Math.max(0, lastSlot - firstSlot + 1) }, (_, offset) => {
    const centerTime = (firstSlot + offset + 0.5) * slotSeconds;
    return presentationTimestamps?.length ? timestampToFrameIndex(presentationTimestamps, centerTime) : Math.round(centerTime * frameRate);
  }).map((frame) => Math.min(maximumFrame, frame)))];
}

export default function FrameThumbnailStrip(props: FrameThumbnailStripProps) {
  const { projectId, sourceUrl, mediaFingerprint, durationSeconds, frameRate, presentationTimestamps, presentationDurations, pixelsPerSecond, visibleStart, visibleEnd, currentTime, onSeek } = props;
  const requestedFrames = useMemo(() => frameSlots({ durationSeconds, frameRate, presentationTimestamps, pixelsPerSecond, visibleStart, visibleEnd }), [durationSeconds, frameRate, presentationTimestamps, pixelsPerSecond, visibleEnd, visibleStart]);
  ...
  const thumbnails = useFrameThumbnailQueue({ projectId, sourceUrl, mediaFingerprint, durationSeconds, frameRate, presentationTimestamps, presentationDurations, frames: requestedFrames, priorityFrames });
  ...
  return <div className="absolute inset-0 overflow-hidden">
    {requestedFrames.map((frame) => {
      const centerTime = presentationTimestamps?.[frame] ?? frame / frameRate;
