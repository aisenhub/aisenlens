import { ChevronDown, ChevronRight, Filter, MoreHorizontal, Play, Scissors, Search, Trash2, X } from "lucide-react";
...
function ShotList({ className, shots, groups, collapsedGroupIds, activeShotIndex, filters, onFiltersChange, completionByShotId, selectedShotIds, isSelectingShots, shotNotes, shotFields, screenshotIdsByShotId, primaryScreenshotIdsByShotId, markers, frameRate, onLocateShot, onSelectionChange, onToggleGroup, onSelectGroup, onPlayGroup, onDeleteGroup, onPlayShot, onDeleteShot, manualSplitDisabledReason, onSplitAtPlayhead }: ShotListProps) {
  ...
  const groupByFirstShotId = useMemo(() => new Map(groups.map((group) => [group.shotIds[0], group])), [groups]);
  const groupByShotId = useMemo(() => new Map(groups.flatMap((group) => group.shotIds.map((shotId) => [shotId, group] as const))), [groups]);
  const matchingShotIds = useMemo(() => findMatchingShotIds({ shots, groups, notesByShotId: shotNotes, fieldsByShotId: shotFields, screenshotIdsByShotId, primaryScreenshotIdsByShotId, markers, frameRate }, filters), [filters, frameRate, groups, markers, primaryScreenshotIdsByShotId, screenshotIdsByShotId, shotFields, shotNotes, shots]);
    ...
    <div className="flex-1 overflow-y-auto">
      {shots.map((shot, index) => {
        ...
        return <div key={shot.id}>
          {isGroupStart && group && groupHasMatch && <div style={{ contentVisibility: "auto", containIntrinsicSize: "0 36px" }} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-1 border-b border-accent/15 bg-accent/6 px-2 py-1.5"><Button type="button" variant="ghost" size="icon-xs" aria-label={isCollapsed ? "展开结构" : "折叠结构"} onClick={() => onToggleGroup(group.id)} className="text-accent hover:bg-accent/10">{isCollapsed ? <ChevronRight /> : <ChevronDown />}</Button><Button type="button" variant="ghost" size="sm" onClick={() => onSelectGroup(group.id, index)} title={group.title} className="h-7 min-w-0 justify-start truncate px-1 text-left editor-body text-accent hover:bg-accent/10">{group.title}</Button><DropdownMenu><DropdownMenuTrigger render={<Button type="button" variant="ghost" size="icon-xs" aria-label={`结构 ${group.title} 的更多操作`} className="text-accent hover:bg-accent/10" />}><MoreHorizontal /></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-32"><DropdownMenuItem onClick={() => onPlayGroup(group.id)}><Play />播放结构</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => onDeleteGroup(group.id)}><Trash2 />删除结构</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>}
          {(!isCollapsed || isFiltering) && <div style={{ contentVisibility: "auto", containIntrinsicSize: "0 40px" }} ref={(element) => { if (element) shotRowRefs.current.set(shot.id, element); else shotRowRefs.current.delete(shot.id); }} className={`grid ${isSelectingShots ? "grid-cols-[auto_minmax(0,1fr)_auto]" : "grid-cols-[minmax(0,1fr)_auto]"} items-center gap-1 border-b border-border/50 px-3 py-2 transition-colors ${isActive ? "border-l-2 border-l-accent bg-accent/15" : "hover:bg-white/4"}`}>
            {isSelectingShots && <input type="checkbox" checked={isSelected} onChange={(event) => onSelectionChange(shot.id, event.target.checked)} aria-label={`选择分镜 ${index + 1}`} className="editor-checkbox mr-0.5" />}
            <Button type="button" variant="ghost" size="sm" onClick={() => onLocateShot(index)} aria-current={isActive ? "true" : undefined} className="h-8 min-w-0 justify-start gap-1.5 px-0.5 text-left hover:bg-transparent"><span className={`size-1.5 shrink-0 rounded-full ${isComplete ? "bg-emerald-400" : "bg-amber-400"}`} /><span className={`shrink-0 font-mono editor-body ${isActive ? "text-accent" : "text-text-muted"}`}>#{String(index + 1).padStart(2, "0")}</span><span className="min-w-0 truncate editor-micro text-text-muted">已填 {completion.filled}/{completion.total}</span></Button>
            <DropdownMenu><DropdownMenuTrigger render={<Button type="button" variant="ghost" size="icon-xs" aria-label={`分镜 ${index + 1} 的更多操作`} className="text-text-muted hover:bg-white/8 hover:text-white" />}><MoreHorizontal /></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-32"><DropdownMenuItem onClick={() => onPlayShot(index)}><Play />播放分镜</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => onDeleteShot(index)}><Trash2 />删除分镜</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
          </div>}
        </div>;
