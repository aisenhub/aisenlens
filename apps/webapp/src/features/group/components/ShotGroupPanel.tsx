...
export default function ShotGroupPanel({ isSelecting, selectedShotCount, kind, onKindChange, onStartSelection, onCancelSelection, onCreate }: ShotGroupPanelProps) {
  ...
  return <section className="mt-4 border-t border-border pt-3">
    ...
    <div className="mt-3 rounded-xl border border-border bg-bg-deep p-2.5">
      ...
      {isSelecting ? <><p className="mt-2 editor-meta text-text-muted">已选 {selectedShotCount} 个连续镜头 · 场景可只包含一个镜头</p><div className="mt-2 grid grid-cols-[auto_minmax(0,1fr)] gap-1"><Button type="button" variant="ghost" size="xs" onClick={onCancelSelection} className="shrink-0 editor-body font-normal text-text-muted">取消</Button><Button type="button" size="xs" disabled={!canCreate} onClick={onCreate} className="min-w-0 px-1.5 editor-body font-normal"><FolderPlus className="shrink-0" />确认（{selectedShotCount}）</Button></div></> : <Button type="button" variant="outline" size="sm" onClick={onStartSelection} className="mt-2 h-7 w-full editor-body font-normal border-accent/30 text-accent hover:bg-accent/10"><FolderPlus />选择镜头创建分组</Button>}
    </div>
  </section>;
}
