...
implementation_areas:
  - apps/webapp/src/features/timeline
  - apps/webapp/src/features/shot
  - apps/webapp/src/features/video
...
STRUCTURE
  Scene         26–32px
  Sequence      22–26px  ┐
  Section       20–24px  ┘ 可折叠为 Macro Structure
...
微观在上、宏观在下：Shot + Frame 位于最上方，结构轨按 Scene → Sequence → Section 依次向下展开；Shot + Frame 视觉权重最高。尺寸是初始设计范围，实际交互命中区须独立于细线宽度。Macro Structure 折叠后保留摘要与展开入口。
...
研究文件与确认事实应继续以仓库 `reference-projects/REFERENCE_PROJECT_INDEX.md` 为外部依据；该文件不属于本次输入压缩包，因此这里保留仓库路径而不建立失效的包内相对链接。本节只说明设计采用范围。
