...
- Phase 01 已满足“已交付”。
- Marker v2 / DB v18 / Structure validator 已 push。
...
Registry 是 label/category/renderMode/defaults/min/max 的唯一 source；`EditorTimeline` 不再维护第二份 label map。
...
Marker / 未来 Analysis / Audio 可按产品允许规则调整，但不能把 Scene 拖到 Section 上方形成反向 hierarchy。
...
- Shot click → active + seek shot start。
- Frame click/strip seek。
- playhead。
- horizontal scroll。
- zoom/pan。
