...
本记录仍不把 reduced-motion、真实 Windows IME composition、真实浏览器 200% 菜单缩放、断网/权限弹窗注入写成通过；四尺寸和 200% 等效视口验收明确存在固定多栏裁切问题，长片视图切换存在性能风险。现有自动分镜和视频导出专项测试已按原脚本独立运行；构建保留既有 WASM 外部化与大 chunk 警告。
