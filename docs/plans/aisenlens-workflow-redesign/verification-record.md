...
- 自动分镜复核入口：准备阶段扫描完成后结果区仅显示检测边界数与候选段数；“重新扫描”继续留在准备阶段，“应用分镜”导航到校准阶段，候选队列、排除选择和应用预览统一在校准阶段处理。
- Analyze 布局：修复 Scenes 内容容器未建立 flex 高度上下文导致的视频区按内容高度收缩问题；视频预览与时间线现在共同占满阶段内容区，空素材状态也保持完整可见。
