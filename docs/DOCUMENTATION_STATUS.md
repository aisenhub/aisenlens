...
1. `AGENTS.md`：仓库级工程和安全约束。
2. `docs/PROJECT_ARCHITECTURE.md`：当前代码架构、数据边界和依赖职责。
3. `docs/OPERATIONS.md`：当前发布单元、外部平台门禁和线上验证。
4. `docs/SEO_DISCOVERABILITY_PLAN.md`：公开站 SEO、索引和 canonical 事实。
5. `docs/DEVELOPMENT_TODO.md`：当前后续工作，不代表任务已实现。
6. `docs/auto-shot/README.md` 及其索引下的长期引擎文档：自动分镜生产基线与研究范围。
7. `docs/plans/**` 与 `docs/chaifen/**`：阶段计划和执行记录；以其中最新的“当前事实/验证记录”章节为准。
8. `reference-projects/**` 与 `docs/**/reference/**`：仅为参考项目或历史方案资料，不是当前实现来源。

## 拆分执行包的当前状态

`docs/chaifen/AISENLENS_WEB_SPLIT_AGENT_EXECUTION_PACKAGE/aisenlens_web_split_agent_execution/`
...
该执行包中的 `/reset-password`、`/support`、`/feedback`、Auth 和 Supabase
allow-list 流程属于原始方案中的条件分支；它们不属于当前代码范围。原始方案中的
`apps/web`、`@aisenlens/web` 和 `apps/web/dist` 只在“拆分前基线”语境下保留，不能作为当前路径使用。

## 文档审计规则

- 本次审计扫描仓库内 82 个 Markdown/MDX 文件。
- 已更新根 README、架构、运维、SEO、开发待办、Web 审计快照、拆分执行包的当前状态和本索引。
