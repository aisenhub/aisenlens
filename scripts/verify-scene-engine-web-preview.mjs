...
async function serveStatic(port) {
  const fixturePath = resolve(repositoryDirectory, "apps/web/test/test.mov")
  await mkdir(resolve(outputDirectory, "test"), { recursive: true })
  await copyFile(fixturePath, resolve(outputDirectory, "test/test.mov"))
...
try {
  ...
  const expression = `(async () => new Promise(async (resolve) => { const source = await fetch('${baseUrl}test/test.mov').then((response) => response.blob()); const worker = new Worker('${workerUrl}', { type: 'module' }); const messages = []; const timer = setTimeout(() => { worker.terminate(); resolve({ status: 'timeout', messages }); }, 120000); worker.onmessage = (event) => { messages.push(event.data); if (event.data.type === 'READY') worker.postMessage({ type: 'START', jobId: 'preview-smoke-1', source, mediaFingerprint: 'sha256:preview-smoke', config: { hardCut: { kind: 'content', threshold: 2700, weights: { hue: 3333, saturation: 3333, luma: 3334 } }, fade: null, minimumSceneDurationUs: 600000, analysis: { maxWidth: 96, temporalSampling: { kind: 'every-frame' } }, diagnostics: 'off' } }); if (event.data.type === 'PROGRESS' && event.data.progress.decodedFrames >= 12) worker.postMessage({ type: 'PAUSE', jobId: 'preview-smoke-1' }); if (event.data.type === 'CHECKPOINT' || event.data.type === 'ERROR') { clearTimeout(timer); worker.terminate(); resolve({ status: event.data.type === 'CHECKPOINT' ? 'paused' : 'error', messages }); } }; worker.onerror = (event) => { clearTimeout(timer); worker.terminate(); resolve({ status: 'runtime-error', messages, error: event.message }); }; worker.postMessage({ type: 'INIT' }); }))()`.replace(/\\/g, "\\\\")
  ...
  await writeFile(resolve(repositoryDirectory, "test-results", "scene-engine-preview-smoke.json"), JSON.stringify({ generatedAt: new Date().toISOString(), browser: version.Browser, base: manifest.base, worker: manifest.workerFiles[0], backend: ready.backend, version: ready.version, ...smoke }, null, 2), "utf8")
  assert.equal(smoke.status, "paused", JSON.stringify(smoke))
  console.log(JSON.stringify({ status: smoke.status, backend: ready.backend, version: ready.version, base: manifest.base, worker: manifest.workerFiles[0], wasm: manifest.wasmFiles[0], messageTypes: smoke.messages.map((message) => message.type) }, null, 2))
} finally {
  client?.close()
